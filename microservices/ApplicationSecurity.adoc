...
.. For non-idempotent requests (i.e. POST requests) that are less than 5
   minutes old, the **gameon-signature** value should be compared to recently
   seen requests to prevent replays.

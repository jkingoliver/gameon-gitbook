...
Game On! is an application composed of several microservices. The set services h
as changed over time: the best practice with microservice architectures is still
...
use an external data store. As noted abouve, its API is a {websocket}[websocket
...
The Map service is implemented in Java and run on WAS Liberty. It stores data in
...
slice this particular onion, but the important for us was being able to see the

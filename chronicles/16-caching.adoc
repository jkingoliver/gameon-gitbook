...
Redisson needs a {redisson-jcache-config}[config file], to tell it how to connect to Redis (amongst other things), and because JSR107 is implementation agnostic,
...
work. (_Update, {redisson-3-2-4}[Redisson 3.2.4] has released with proper support for using Redisson config during JCache cache creation_) I made a few tweaks to the forked CDI annotation layer, so that {bdcache}[when it obtained its default cache manager], it would do so via
my own VCAP_SERVICES configured Redisson.. and bam.. {ppap}[Pen Pineapple Apple Pen.]

## Using all the toys

From that point, I had a working JSR107 implementation to use, and via the magic of {jitpack}[jitpack], I could use it directly as a dependency in

...
Game On Rooms can support the concepts of 'items' via the websocket protocol. This means there is a difference between
...
to Game On. It's this "other information" that we can use to tell the game about items in the room.
...
When the game renders the list in the web interface, it will look something like:
...
TIP: Use unique names for the items in your room so you can tell them apart. In real code, `itemA` and `itemB` might get a little confusing, and consider the challenges of having to tell the difference between `coffee` and `coffee machine`.
...
You might want to consider the Caching adventure, where you can learn about stateful items.
